...
3. **時間區間判定**：
   - 模糊時間（例如：「之前」、「過去」、「上次」）：請直接將 query_start_time 與 query_end_time 設為 null。
   - 包含當下的區間（例如：「這禮拜」、「今天」、「這個月」）：請明確設定 query_start_time（例如這週的第一天 00:00:00），但「務必將 query_end_time 設為 null」，因為設為現在時間可能會導致比對時錯失存入資料庫延遲或時區差異造成的當日最新紀錄。
